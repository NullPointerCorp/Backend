...
export const updateAlmacen = async (almacen_id: number, data: Partial<CrearAlmacenDTO>): Promise<AlmacenDTO> => {
  ...
  if ((result as any).affectedRows === 0) throw new Error("Almacén no encontrado");
  return await findAlmacenById(almacen_id) as AlmacenDTO;
};

export const deleteAlmacen = async (almacen_id: number): Promise<void> => {
  try {
    await pool.query(`DELETE FROM almacenes WHERE almacen_id = ?`, [almacen_id]);
  } catch (error: any) {
