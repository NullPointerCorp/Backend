...
export const getAllTransportesSucursal = async (empleado_id: number) => {
  const [rows] = await pool.query(
    ...
    SELECT
      ...
      t.placa
    FROM transportes t
    INNER JOIN empleados emp_actual
      ON emp_actual.sucursal_id = t.sucursal_id
    LEFT JOIN empleados e
      ON t.empleado_id = e.empleado_id
    INNER JOIN subtipo_transporte st
      ON t.subtipo_id = st.subtipo_id
    INNER JOIN tipo_transporte tt
      ON st.tipo_id = tt.tipo_id
    WHERE emp_actual.empleado_id = ?
...
export const createTransporte = async (data: CrearTransporteDTO) => {
  await pool.query(
    `INSERT INTO transportes (numero_serie, sucursal_id, empleado_id, subtipo_id, capacidad_carga, unidad_medida, placa)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [
      data.numero_serie,
      data.sucursal_id,
...
export const updateTransporte = async (
  ...
  if (result.affectedRows === 0) {
    throw new NotFoundError("Transporte no encontrado");
  }
